...
import { File as AtomFile, Range as AtomRange, CompositeDisposable } from "atom";
import RFC, { Configs, EditorState } from "./read-from-configcson";
import { getExtension, getOnlySecond } from "./utilities/utility";
...
export default {
  ...
  initalize() {
    ...
    this.kInitRange = new AtomRange([Infinity, 0], [0, 0]);
  ...
  */
  mark(editorState, pattern, range) {
    ...
    for (let patternFiletype of pattern.fileTypes) {
      for (let editorFiletype of editorState.fileTypes) {
        if (patternFiletype === "*" || patternFiletype === editorFiletype) { // pattern.fileTypes と editorState.fileTypesには必ず値が入っている
  ...
  getChangedRange(evt) {
    ...
    this.changedRange = new AtomRange([minR, 0], [maxR, 0]);
    return this.changedRange;
  },

  //+1は問題ないが、-1は0以下にしてはいけない
  getMarkRange(range, lastRow) {
    let markRange = new AtomRange([0, 0], [0, 0]);
  ...
  getTextEditors() {
    return (editor) => {
      ...
      editorState.fileTypes = [getExtension(editor.getPath())]; //add file extension.
      editorState.fileTypes = editorState.fileTypes.concat(editor.getGrammar().fileTypes);
  ...
  */
  ...
  setFileInvalid() {
    return (err) => {
      if (err[1] === 0) {
        atom.notifications.addError("Cannot read file from patternsFilePath",
          { "detail": err[0], "dismissable": true });
        this.fileDispose();
      } else if (err[1] === 1) {
        atom.notifications.addError("patternsFile is not valid cson file",
          { "detail": err[0], "dismissable": true });
      }
    };
  },
  ...
  readPatternsFile() {
    let oldCache = ""; //FIXME var to let. need test
